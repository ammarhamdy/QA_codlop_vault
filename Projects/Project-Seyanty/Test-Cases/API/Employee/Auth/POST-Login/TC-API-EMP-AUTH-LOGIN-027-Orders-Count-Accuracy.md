---
tc_id: TC-API-EMP-AUTH-LOGIN-027
...
Verify `orders_count` equals length of `orders` array and matches DB count.
...
# Preconditions
- Employee-09 has 0 orders per sample.
...
# Steps
1. Login.
2. Extract `orders_count` and `orders|length`.
3. Assert equality.
4. If employee has orders, both should be >0 and equal.

# Expected Result
- `data.orders_count` is integer.
- `data.orders_count` == `data.orders | length`.
- For seed employee-09, both `0` and `[]`.
...
```bash
#!/usr/bin/env bash
# Test Case: TC-API-EMP-AUTH-LOGIN-027 - Orders Count Accuracy
# Endpoint: POST https://seyanty.info/api/employee/login
TITLE="TC-API-EMP-AUTH-LOGIN-027: Orders Count Accuracy"
echo "=================================================="
echo "Running: $TITLE"
echo "=================================================="
