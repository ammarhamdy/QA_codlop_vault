---
tc_id: TC-API-EMP-AUTH-LOGIN-024
...
Verify returned employee data matches stored DB record for that employee.
...
# Preconditions
- Seeded employee 63: id 63, name employee-09, email employee-09@mail.com, phone 0500000109, job_title engineer.
...
3. Compare to known seed via subsequent GET profile or DB query.
4. Assert phone/job_title/photo match.

# Expected Result
- `data.id` = 63.
- `data.name` = `employee-09`.
- `data.email` = `employee-09@mail.com`.
- `data.phone` = `0500000109`.
- `data.job_title` = `engineer`.
- `data.photo` = `https://seyanty.info/storage/employees/zy0RHyVmKiz1KoWP9jsxRfrIpd4qL8BRNLJ0hRkA.jpg` (or valid URL pattern).
...
# Notes
Cross-check via GET /api/employee/profile with token.

# Attachments/Script
```bash
#!/usr/bin/env bash
# Test Case: TC-API-EMP-AUTH-LOGIN-024 - Employee Data Accuracy
# Endpoint: POST https://seyanty.info/api/employee/login
TITLE="TC-API-EMP-AUTH-LOGIN-024: Employee Data Accuracy"
echo "=================================================="
echo "Running: $TITLE"
echo "=================================================="
