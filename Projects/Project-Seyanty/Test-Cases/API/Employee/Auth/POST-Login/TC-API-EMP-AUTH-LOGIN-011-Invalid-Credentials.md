---
tc_id: TC-API-EMP-AUTH-LOGIN-011
...
Verify combined invalid identifier and invalid password returns generic 401 error.
...
| email_or_name | `nonexistent@mail.com` |
| password | `BadPass123!` |

# Preconditions
- Neither identifier nor password are valid.
...
curl --silent --write-out "\nHTTP_STATUS:%{http_code}" --location --request POST 'https://seyanty.info/api/employee/login' \
  ...
  --form 'email_or_name="nonexistent@mail.com"' \
  --form 'password="BadPass123!"'
  ```

# Steps
1. POST with fully invalid credentials.
2. Assert generic error, no detail leakage.

# Expected Result
- HTTP `401`.
- Same generic schema as TC-010.
- No distinction which field was wrong.
...
# Notes
Security: must not reveal which credential was wrong.

# Attachments/Script
```bash
#!/usr/bin/env bash
# Test Case: TC-API-EMP-AUTH-LOGIN-011 - Invalid Credentials
# Endpoint: POST https://seyanty.info/api/employee/login
TITLE="TC-API-EMP-AUTH-LOGIN-011: Invalid Credentials"
echo "=================================================="
echo "Running: $TITLE"
echo "=================================================="
