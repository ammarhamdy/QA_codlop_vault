---
tc_id: TC-API-EMP-AUTH-LOGIN-010
...
Verify login fails when password is incorrect for a valid identifier.
...
# Preconditions
- Employee `employee-09@mail.com` exists with password `Admin#123`.
...
curl --silent --write-out "\nHTTP_STATUS:%{http_code}" --location --request POST 'https://seyanty.info/api/employee/login' \
  ...
  --form 'password="WrongPass#999"'
  ```

# Steps
1. Send POST with correct email but wrong password.
2. Verify 401.
3. Ensure no token.

# Expected Result
- HTTP `401`.
- `status` error, `code` 401, message `البيانات خاطئه`.
- `data` null.
...
# Notes
Password check must be case-sensitive and exact.

# Attachments/Script
```bash
#!/usr/bin/env bash
# Test Case: TC-API-EMP-AUTH-LOGIN-010 - Incorrect Password
# Endpoint: POST https://seyanty.info/api/employee/login
TITLE="TC-API-EMP-AUTH-LOGIN-010: Incorrect Password"
echo "=================================================="
echo "Running: $TITLE"
echo "=================================================="
...
RESPONSE=$(curl --silent --write-out "\nHTTP_STATUS:%{http_code}" --location --request POST 'https://seyanty.info/api/employee/login' \
  ...
  --header 'Connection: keep-alive' \
  --form 'email_or_name="employee-09@mail.com"' \
  --form 'password="WrongPass#999"' 2>&1)
