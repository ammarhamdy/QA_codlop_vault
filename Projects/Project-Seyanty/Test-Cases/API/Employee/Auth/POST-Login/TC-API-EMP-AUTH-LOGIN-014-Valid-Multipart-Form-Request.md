---
tc_id: TC-API-EMP-AUTH-LOGIN-014
title: Valid Multipart Form Request
priority:
  - High
status:
  - completed
...
Verify endpoint correctly handles `multipart/form-data` with explicit boundary (as in sample Apidog request).
...
| ----- | ----- |
| Endpoint | `POST https://seyanty.info/api/employee/login` |
| Content-Type | `multipart/form-data; boundary=--------------------------447325079108028205023565` |
| email_or_name | `employee-09@mail.com` |
| password | `Admin#123` |

# Preconditions
- Valid credentials.
- Tool supports multipart boundary.
...
curl --silent --write-out "\nHTTP_STATUS:%{http_code}" --location --request POST 'https://seyanty.info/api/employee/login' \
  ...
  --header 'Content-Type: multipart/form-data; boundary=--------------------------447325079108028205023565' \
  --form 'email_or_name="employee-09@mail.com"' \
  --form 'password="Admin#123"'
  ```

# Steps
1. Send exact Apidog sample: multipart with boundary header.
2. Verify 200 and data.
3. Repeat with auto-boundary via --form only to confirm both succeed.

# Expected Result
- HTTP `200`.
- Success schema identical to TC-001.
- Server parses multipart correctly regardless of boundary auto vs manual.
...
# Notes
This is the canonical request format from spec; JSON body should not be assumed.

# Attachments/Script
```bash
#!/usr/bin/env bash
# Test Case: TC-API-EMP-AUTH-LOGIN-014 - Valid Multipart Form Request
# Endpoint: POST https://seyanty.info/api/employee/login
TITLE="TC-API-EMP-AUTH-LOGIN-014: Valid Multipart Form Request"
echo "=================================================="
echo "Running: $TITLE"
echo "=================================================="
...
RESPONSE=$(curl --silent --write-out "\nHTTP_STATUS:%{http_code}" --location --request POST 'https://seyanty.info/api/employee/login' \
  ...
  --header 'Host: seyanty.info' \
  --header 'Connection: keep-alive' \
  --header 'Content-Type: multipart/form-data; boundary=--------------------------102216812367033434231009' \
  --form 'email_or_name="employee-09@mail.com"' \
