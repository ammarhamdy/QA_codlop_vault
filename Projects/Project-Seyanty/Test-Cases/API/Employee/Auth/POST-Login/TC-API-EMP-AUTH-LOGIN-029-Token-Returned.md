---
tc_id: TC-API-EMP-AUTH-LOGIN-029
...
Verify successful login returns a non-empty authentication token in `data.token`.
...
3. Assert exists, type string, length >20, matches JWT pattern `eyJ*`.

# Expected Result
- `data.token` present, non-empty string.
- Looks like JWT (header.payload.signature with dots).
- No token on failed logins.
...
None. Keep token for downstream tests.

# Notes
Token is primary artifact of login.

# Attachments/Script
```bash
#!/usr/bin/env bash
# Test Case: TC-API-EMP-AUTH-LOGIN-029 - Token Returned
# Endpoint: POST https://seyanty.info/api/employee/login
TITLE="TC-API-EMP-AUTH-LOGIN-029: Token Returned"
echo "=================================================="
echo "Running: $TITLE"
echo "=================================================="
