---
tc_id: TC-API-EMP-AUTH-LOGIN-019
title: Invalid Credentials Response Status Code
priority:
  - High
status:
  - completed
...
Assert invalid credentials returns HTTP 401 Unauthorized (not 200/400/500).
...
# Preconditions
- Valid identifier, wrong password.
...
curl --silent --write-out "\nHTTP_STATUS:%{http_code}" --location --request POST 'https://seyanty.info/api/employee/login' \
  ...
  --form 'password="Wrong#123"'
  ```

# Steps
1. POST invalid password.
2. Capture HTTP_STATUS.
3. Assert 401.

# Expected Result
- HTTP `401`.
- Body `code`: 401, status error.
...
# Notes
Error code consistency: HTTP status must equal body `code`.

# Attachments/Script
```bash
#!/usr/bin/env bash
# Test Case: TC-API-EMP-AUTH-LOGIN-019 - Invalid Credentials Response Status Code
# Endpoint: POST https://seyanty.info/api/employee/login
TITLE="TC-API-EMP-AUTH-LOGIN-019: Invalid Credentials Response Status Code"
echo "=================================================="
echo "Running: $TITLE"
echo "=================================================="
