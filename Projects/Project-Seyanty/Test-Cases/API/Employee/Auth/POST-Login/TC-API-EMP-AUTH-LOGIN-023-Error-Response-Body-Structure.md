---
tc_id: TC-API-EMP-AUTH-LOGIN-023
...
type:
  - API
linked_requirement: REQ-EMP-AUTH-LOGIN-006
tags:
  - test-case
  - api
  - employee
...
Validate error response JSON schema for invalid credentials/missing fields.
...
| ----- | ----- |
| Endpoint | `POST https://seyanty.info/api/employee/login` |
| email_or_name | `not-exists@mail.com` |
| password | `BadPass` |

# Preconditions
- Invalid credentials to trigger error.
...
curl --silent --write-out "\nHTTP_STATUS:%{http_code}" --location --request POST 'https://seyanty.info/api/employee/login' \
  ...
  --form 'email_or_name="not-exists@mail.com"' \
  --form 'password="BadPass"'
  ```

# Steps
1. POST invalid to force error.
2. Validate error schema.
3. Check no success keys.

# Expected Result
- HTTP 401.
- Body: `{"status":"error","code":401,"message":"البيانات خاطئه","data":null}` exactly.
- Types: `status` string, `code` int, `message` string (Arabic), `data` null.
...
```bash
#!/usr/bin/env bash
# Test Case: TC-API-EMP-AUTH-LOGIN-023 - Error Response Body Structure
# Endpoint: POST https://seyanty.info/api/employee/login
TITLE="TC-API-EMP-AUTH-LOGIN-023: Error Response Body Structure"
echo "=================================================="
echo "Running: $TITLE"
echo "=================================================="
