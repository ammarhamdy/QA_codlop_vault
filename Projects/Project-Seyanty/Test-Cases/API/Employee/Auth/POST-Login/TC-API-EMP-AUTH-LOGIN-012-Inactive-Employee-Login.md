---
tc_id: TC-API-EMP-AUTH-LOGIN-012
title: Inactive Employee Login
priority:
  - Medium
status:
  - completed
type:
  - API
linked_requirement: REQ-EMP-AUTH-LOGIN-007
tags:
  - test-case
  - api
  - employee
...
Verify behavior when an inactive-status employee attempts to login (sample shows `status: inactive` still returns success).
...
| email_or_name | `employee-09@mail.com` (inactive) |
| password | `Admin#123` |

# Preconditions
- Employee-09 has `status: inactive` per sample (or explicitly deactivated employee).
...
2. Observe whether API allows login or returns error.
3. Document actual vs expected per business rule.

# Expected Result
- **Current implementation:** HTTP `200` with `status: inactive` in `data.status` and valid token (as per provided success sample).
- If business rule requires blocking inactive users, expected alternative is `401` with `البيانات خاطئه` or inactive-specific message.
- This test records actual behavior; mark as `pass` if 200 with inactive status, `fail` if business decides to block.
...
# Notes
Sample response shows `status: inactive` but `200 success` – spec unclear. This TC validates/ documents that behavior.

# Attachments/Script
```bash
#!/usr/bin/env bash
# Test Case: TC-API-EMP-AUTH-LOGIN-012 - Inactive Employee Login
# Endpoint: POST https://seyanty.info/api/employee/login
TITLE="TC-API-EMP-AUTH-LOGIN-012: Inactive Employee Login"
echo "=================================================="
echo "Running: $TITLE"
echo "=================================================="
