---
tc_id: TC-API-EMP-AUTH-LOGIN-028
...
Verify `orders` is empty array `[]` (not null or missing) when employee has no orders.
...
# Steps
1. Login.
2. Assert `data.orders` is array.
3. Assert length 0.
4. Ensure iterable without null error.

# Expected Result
- `data.orders` is `[]` (empty array).
- Type `array`, not `null`.
...
# Notes
Empty collection should be array, not null.

# Attachments/Script
```bash
#!/usr/bin/env bash
# Test Case: TC-API-EMP-AUTH-LOGIN-028 - Empty Orders List Handling
# Endpoint: POST https://seyanty.info/api/employee/login
TITLE="TC-API-EMP-AUTH-LOGIN-028: Empty Orders List Handling"
echo "=================================================="
echo "Running: $TITLE"
echo "=================================================="
