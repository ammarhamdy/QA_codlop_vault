---
tc_id: TC-API-EMP-AUTH-LOGIN-026
...
Verify `overview` field correctly returns `null` when no overview is set, without breaking schema.
...
# Preconditions
- Employee-09 has overview = null (per sample).
...
# Steps
1. Login.
2. Check `data.overview` is `null` (not missing, not "", not "null" string).
3. Validate type via `jq '.data.overview == null'`.

# Expected Result
- `data.overview` is JSON `null`.
- Field exists (key present).
- No error when overview null.
...
```bash
#!/usr/bin/env bash
# Test Case: TC-API-EMP-AUTH-LOGIN-026 - Null Overview Handling
# Endpoint: POST https://seyanty.info/api/employee/login
TITLE="TC-API-EMP-AUTH-LOGIN-026: Null Overview Handling"
echo "=================================================="
echo "Running: $TITLE"
echo "=================================================="
