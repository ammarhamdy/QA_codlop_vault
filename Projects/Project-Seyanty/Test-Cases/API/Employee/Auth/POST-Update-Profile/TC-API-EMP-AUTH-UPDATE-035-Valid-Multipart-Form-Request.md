---
tc_id: TC-API-EMP-AUTH-UPDATE-035
...
Verify endpoint correctly handles multipart/form-data with explicit boundary as in sample.
...
# Preconditions
- Valid token.
- Tool supports explicit boundary.
...
- **Notes:** Send exact boundary header as in sample; curl --form auto-generates but we also set explicit Content-Type to emulate Apidog.
...
curl --silent --write-out "\nHTTP_STATUS:%{http_code}" --location --request POST 'https://seyanty.info/api/employee/update-profile' \
  ...
  --header 'Content-Type: multipart/form-data; boundary=--------------------------918253417226265533759659' \
...
# Steps
1. Obtain token.
2. POST with explicit boundary header and --form fields.
3. Verify 200 and data.

# Expected Result
- HTTP `200`.
- Success schema identical to TC-001.
- Server parses multipart regardless of boundary auto vs manual.
...
# Notes
Canonical request format from spec.

# Attachments/Script
```bash
#!/usr/bin/env bash
# Test Case: TC-API-EMP-AUTH-UPDATE-035 - Valid Multipart Form Request
# Endpoint: POST https://seyanty.info/api/employee/update-profile
TITLE="TC-API-EMP-AUTH-UPDATE-035: Valid Multipart Form Request"
echo "=================================================="
echo "Running: $TITLE"
echo "=================================================="
