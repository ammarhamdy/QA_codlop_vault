---
tc_id: TC-API-EMP-AUTH-UPDATE-038
...
type:
  - API
linked_requirement: REQ-EMP-AUTH-UPDATE-018
tags:
  - test-case
  - api
  - employee
...
Verify GET method is rejected – only POST is allowed. Spec lists GET erroneously, sample uses POST.
...
| Field | Value |
| ----- | ----- |
| Endpoint | `GET https://seyanty.info/api/employee/update-profile` (invalid) |
| Method | `GET` |
...
- **Notes:** Send GET to same URL with Authorization and query params.
...
curl --silent --write-out "\nHTTP_STATUS:%{http_code}" --location --request GET 'https://seyanty.info/api/employee/update-profile?name=employee-09&email=employee-09@mail.com' \
...
# Steps
1. Obtain token.
2. Send GET with query params and Authorization.
3. Verify 405.
4. Confirm POST with same token returns 200.

# Expected Result
- HTTP `405 Method Not Allowed` (or `404` if not routed).
- No update, no token.
...
# Notes
Spec header says GET but sample uses POST – POST is authoritative; GET must fail.

# Attachments/Script
```bash
#!/usr/bin/env bash
# Test Case: TC-API-EMP-AUTH-UPDATE-038 - Invalid HTTP Method
# Endpoint: POST https://seyanty.info/api/employee/update-profile
TITLE="TC-API-EMP-AUTH-UPDATE-038: Invalid HTTP Method"
echo "=================================================="
echo "Running: $TITLE"
echo "=================================================="
