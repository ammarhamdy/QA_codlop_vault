---
tc_id: TC-API-EMP-AUTH-UPDATE-004
title: Update Phone
priority:
  - Medium
status:
type:
  - API
linked_requirement: REQ-EMP-AUTH-UPDATE-004
tags:
  - test-case
  - api
  - employee
...
Verify `phone` field update persists correctly.
...
# Preconditions
- Valid token.
- New phone `0500000111` valid Saudi format.
...
# Steps
1. Login.
2. POST with `phone=0500000111`.
3. Verify `data.phone` updated.
4. GET profile check.

# Expected Result
- HTTP `200`.
- `data.phone == "0500000111"` string.
...
Revert phone to `0500000109`.

# Notes
Phone format must remain string, not integer.

# Attachments/Script
```bash
#!/usr/bin/env bash
# Test Case: TC-API-EMP-AUTH-UPDATE-004 - Update Phone
# Endpoint: POST https://seyanty.info/api/employee/update-profile
TITLE="TC-API-EMP-AUTH-UPDATE-004: Update Phone"
echo "=================================================="
echo "Running: $TITLE"
echo "=================================================="
