---
tc_id: TC-API-EMP-AUTH-UPDATE-050
...
Ensure password (or hash) is never returned in update success or error response.
...
| password sent | `Admin#123` must NOT appear in response |
...
- **Notes:** Valid update with --include to capture headers/body for leak check.
...
# Steps
1. POST valid update with --include.
2. Search for `password` case-insensitively in full response.
3. Check JSON keys via `jq '.data|keys'` no `password`.

# Expected Result
- No `password` or `password_hash` key in `data`.
- No plaintext `Admin#123` in body.
...
```bash
#!/usr/bin/env bash
# Test Case: TC-API-EMP-AUTH-UPDATE-050 - Password Not Exposed In Response
# Endpoint: POST https://seyanty.info/api/employee/update-profile
TITLE="TC-API-EMP-AUTH-UPDATE-050: Password Not Exposed In Response"
echo "=================================================="
echo "Running: $TITLE"
echo "=================================================="
