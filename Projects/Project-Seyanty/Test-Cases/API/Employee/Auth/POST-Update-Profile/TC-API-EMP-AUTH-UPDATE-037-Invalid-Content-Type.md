---
tc_id: TC-API-EMP-AUTH-UPDATE-037
...
- **Notes:** Send JSON body with `Content-Type: application/json`.
...
curl --silent --write-out "\nHTTP_STATUS:%{http_code}" --location --request POST 'https://seyanty.info/api/employee/update-profile' \
  ...
  --data '{"name":"employee-09","email":"employee-09@mail.com","password":"Admin#123","phone":"0500000109","job_title":"eng","overview":"experienced engineer"}'
  ```

# Steps
1. Obtain token.
2. Send JSON body with application/json.
3. Check response.

# Expected Result
- Two acceptable: `200` if tolerant (Laravel often accepts both), or `400`/`422`/`415` with error if strict multipart required. Must not be `500`.
...
# Notes
Spec says multipart but many APIs accept both; covers tolerance.

# Attachments/Script
```bash
#!/usr/bin/env bash
# Test Case: TC-API-EMP-AUTH-UPDATE-037 - Invalid Content Type
# Endpoint: POST https://seyanty.info/api/employee/update-profile
TITLE="TC-API-EMP-AUTH-UPDATE-037: Invalid Content Type"
echo "=================================================="
echo "Running: $TITLE"
echo "=================================================="
...
curl --silent --write-out "\nHTTP_STATUS:%{http_code}" --location --request POST 'https://seyanty.info/api/employee/update-profile' \
  ...
  --header 'Content-Type: application/json' \
  --header 'Host: seyanty.info' \
  --data '{"name":"employee-09","email":"employee-09@mail.com","password":"Admin#123","phone":"0500000109","job_title":"eng","overview":"experienced engineer"}' 2>&1)
