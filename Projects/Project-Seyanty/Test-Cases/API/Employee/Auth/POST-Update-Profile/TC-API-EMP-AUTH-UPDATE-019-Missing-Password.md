---
tc_id: TC-API-EMP-AUTH-UPDATE-019
...
type:
  - API
linked_requirement: REQ-EMP-AUTH-UPDATE-011
tags:
  - test-case
  - api
  - employee
...
Verify behavior when `password` is omitted (is password required on update?).
...
- **Notes:** Omit password field – many APIs make password optional on update.
...
# Steps
1. Login.
2. POST omitting password.
3. Observe 200 (if optional) or 422 (if required).

# Expected Result
- If optional: HTTP `200` with other fields updated and password unchanged. If required: HTTP `422` with password required message. Document actual.
...
# Notes
Password may be optional on profile update – test documents contract.

# Attachments/Script
```bash
#!/usr/bin/env bash
# Test Case: TC-API-EMP-AUTH-UPDATE-019 - Missing Password
# Endpoint: POST https://seyanty.info/api/employee/update-profile
TITLE="TC-API-EMP-AUTH-UPDATE-019: Missing Password"
echo "=================================================="
echo "Running: $TITLE"
echo "=================================================="
