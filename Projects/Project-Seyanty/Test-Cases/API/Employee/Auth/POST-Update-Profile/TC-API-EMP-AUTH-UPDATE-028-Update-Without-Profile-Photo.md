---
tc_id: TC-API-EMP-AUTH-UPDATE-028
title: Update Without Profile Photo
priority:
  - Medium
status:
type:
  - API
linked_requirement: REQ-EMP-AUTH-UPDATE-015
tags:
  - test-case
  - api
  - employee
...
Verify profile can be updated without sending `photo` field (photo optional, should retain old photo).
...
- **Notes:** Omit `photo` field entirely; send only text fields.
...
# Capture old photo
OLD_PHOTO=$(curl --silent --location --request GET 'https://seyanty.info/api/employee/profile' \
  --header "Authorization: Bearer $TOKEN" \
  --header 'Accept: application/json' | jq -r '.data.photo')
echo "OLD_PHOTO:$OLD_PHOTO"
...
# Steps
1. Get old photo via GET profile.
2. Login, POST update without photo field.
3. Verify 200 and `data.photo == OLD_PHOTO` (unchanged).

# Expected Result
- HTTP `200`.
- Photo remains same as before (not nulled).
...
# Notes
Photo optional – omission should not delete.

# Attachments/Script
```bash
#!/usr/bin/env bash
# Test Case: TC-API-EMP-AUTH-UPDATE-028 - Update Without Profile Photo
# Endpoint: POST https://seyanty.info/api/employee/update-profile
TITLE="TC-API-EMP-AUTH-UPDATE-028: Update Without Profile Photo"
echo "=================================================="
echo "Running: $TITLE"
echo "=================================================="
