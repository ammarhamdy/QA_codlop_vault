---
tc_id: TC-API-EMP-AUTH-UPDATE-049
title: Updated Profile Available Through Profile Endpoint
priority:
  - High
status:
type:
  - API
linked_requirement: REQ-EMP-AUTH-UPDATE-024
tags:
  - test-case
  - api
  - employee
...
Verify updated data is immediately available via GET profile using current valid token.
...
| Updated job_title | `verified_eng` |

# Preconditions
- Valid token.
- Can call both endpoints.
...
- **Notes:** Sequence: update job_title to verified_eng, then GET profile.
...
curl --silent --location --request POST 'https://seyanty.info/api/employee/update-profile' \
  ...
  --form 'job_title="verified_eng"' \
  --form 'overview="experienced engineer"' > /tmp/upd_049.json
cat /tmp/upd_049.json | jq .
NEW_TOKEN=$(jq -r '.data.token // "'$TOKEN'"' /tmp/upd_049.json)
if [ "$NEW_TOKEN" = "null" ] || [ -z "$NEW_TOKEN" ]; then NEW_TOKEN=$TOKEN; fi
...
# Steps
1. Login, update `job_title=verified_eng`.
2. GET profile with current token (use new if rotated).
3. Assert `data.job_title == verified_eng` on profile as well.

# Expected Result
- Update returns 200 with `job_title verified_eng`.
- Profile GET also returns `verified_eng`, proving consistency.
...
Revert job_title to `eng`.

# Notes
Cross-endpoint consistency – profile is source of truth.

# Attachments/Script
```bash
#!/usr/bin/env bash
# Test Case: TC-API-EMP-AUTH-UPDATE-049 - Updated Profile Available Through Profile Endpoint
# Endpoint: POST https://seyanty.info/api/employee/update-profile
TITLE="TC-API-EMP-AUTH-UPDATE-049: Updated Profile Available Through Profile Endpoint"
echo "=================================================="
echo "Running: $TITLE"
echo "=================================================="
