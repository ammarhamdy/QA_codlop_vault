---
tc_id: TC-API-EMP-AUTH-UPDATE-003
title: Update Email
priority:
  - High
status:
...
type:
  - API
linked_requirement: REQ-EMP-AUTH-UPDATE-003
tags:
  - test-case
  - api
  - employee
...
Verify updating `email` field to a new unique email persists and remains usable for next login.
...
| email | `employee-09-updated@mail.com` (new) |
| name | `employee-09` |
| password | `Admin#123` |
| phone | `0500000109` |
| job_title | `eng` |
...
# Preconditions
- Valid token.
- New email `employee-09-updated@mail.com` not taken.
...
- **Notes:** Change `email` to unique new address; include other fields.
...
curl --silent --write-out "\nHTTP_STATUS:%{http_code}" --location --request POST 'https://seyanty.info/api/employee/update-profile' \
  ...
  --form 'email="employee-09-updated@mail.com"' \
...
# Steps
1. Login.
2. POST update with new email.
3. Assert `data.email` new value.
4. Optionally login with new email to confirm.

# Expected Result
- HTTP `200`.
- `data.email == "employee-09-updated@mail.com"`.
- Success message.
...
Revert email to `employee-09@mail.com` immediately to avoid breaking subsequent tests.

# Notes
Email uniqueness critical – test must clean up; if revert fails, future logins use new email.

# Attachments/Script
```bash
#!/usr/bin/env bash
# Test Case: TC-API-EMP-AUTH-UPDATE-003 - Update Email
# Endpoint: POST https://seyanty.info/api/employee/update-profile
TITLE="TC-API-EMP-AUTH-UPDATE-003: Update Email"
echo "=================================================="
echo "Running: $TITLE"
echo "=================================================="
