---
tc_id: TC-API-EMP-AUTH-UPDATE-007
...
type:
  - API
linked_requirement: REQ-EMP-AUTH-UPDATE-007
tags:
  - test-case
  - api
  - employee
...
Verify `job_title` update persists.
...
# Preconditions
- Valid token.
- New job_title `senior_eng` valid.
...
curl --silent --write-out "\nHTTP_STATUS:%{http_code}" --location --request POST 'https://seyanty.info/api/employee/update-profile' \
  ...
  --form 'job_title="senior_eng"' \
  --form 'overview="experienced engineer"'
  ```

# Steps
1. Login.
2. POST with `job_title=senior_eng`.
3. Verify `data.job_title == senior_eng`.
...
# Notes
Job title is free text, not enum.

# Attachments/Script
```bash
#!/usr/bin/env bash
# Test Case: TC-API-EMP-AUTH-UPDATE-007 - Update Job Title
# Endpoint: POST https://seyanty.info/api/employee/update-profile
TITLE="TC-API-EMP-AUTH-UPDATE-007: Update Job Title"
echo "=================================================="
echo "Running: $TITLE"
echo "=================================================="
