---
tc_id: TC-API-EMP-AUTH-UPDATE-033
...
Verify endpoint handles malformed Authorization header gracefully.
...
| Authorization | `BearerTokenWithoutSpace <token>` |

# Preconditions
- Valid token to mangle.
...
- **Notes:** Send `Authorization: BearerToken $TOKEN` (no space).
...
# Steps
1. Obtain token.
2. Send with malformed header `BearerToken $TOKEN`.
3. Assert 401.

# Expected Result
- HTTP `401` generic auth error.
...
```bash
#!/usr/bin/env bash
# Test Case: TC-API-EMP-AUTH-UPDATE-033 - Malformed Authorization Header
# Endpoint: POST https://seyanty.info/api/employee/update-profile
TITLE="TC-API-EMP-AUTH-UPDATE-033: Malformed Authorization Header"
echo "=================================================="
echo "Running: $TITLE"
echo "=================================================="
