---
tc_id: TC-API-EMP-AUTH-UPDATE-051
...
type:
  - API
linked_requirement: REQ-EMP-AUTH-UPDATE-025
tags:
  - test-case
  - api
  - employee
...
Verify no sensitive internal fields leaked beyond allowed profile fields.
...
| Allowed keys | `id,name,email,phone,photo,job_title,overview,token,status,orders_count` (plus maybe orders) |
| Forbidden | `password, password_hash, remember_token, secret, pivot` |
...
- **Notes:** Valid update; dump data keys and scan.
...
curl --silent --include --location --request POST 'https://seyanty.info/api/employee/update-profile' \
  ...
  --form 'job_title="eng"' \
  --form 'overview="experienced engineer"' | tee /tmp/upd_051.txt
echo "---KEYS---"
cat /tmp/upd_051.txt | grep -o '{.*}' | tail -n1 | jq '.data | keys' 2>/dev/null || cat /tmp/upd_051.txt | jq '.data | keys' 2>/dev/null
  ```

# Steps
1. POST valid update.
2. List `data` keys.
3. Assert only allowed keys present, no forbidden.
4. Check no stack trace.

# Expected Result
- Allowed keys only: `id, name, email, phone, photo, job_title, overview, token, status, orders_count` (+ maybe `orders`).
- No sensitive fields.
...
# Notes
Broader audit than 050.

# Attachments/Script
```bash
#!/usr/bin/env bash
# Test Case: TC-API-EMP-AUTH-UPDATE-051 - Sensitive Data Not Exposed
# Endpoint: POST https://seyanty.info/api/employee/update-profile
TITLE="TC-API-EMP-AUTH-UPDATE-051: Sensitive Data Not Exposed"
echo "=================================================="
echo "Running: $TITLE"
echo "=================================================="
...
curl --silent --include --location --request POST 'https://seyanty.info/api/employee/update-profile' \
  ...
  --form 'overview="experienced engineer"' | tee /tmp/upd_051.txt
echo "---KEYS---"
cat /tmp/upd_051.txt | grep -o '{.*}' | tail -n1 | jq '.data | keys' 2>/dev/null || cat /tmp/upd_051.txt | jq '.data | keys' 2>/dev/null 2>&1)
