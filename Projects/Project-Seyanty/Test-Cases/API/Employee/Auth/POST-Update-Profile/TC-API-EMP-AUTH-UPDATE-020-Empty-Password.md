---
tc_id: TC-API-EMP-AUTH-UPDATE-020
...
# Steps
1. Login.
2. POST with empty password.
3. Verify 422 if password supplied must be valid, or 200 if empty means no change (depends on impl).

# Expected Result
- Prefer HTTP `422` if empty password rejected; else document if 200 with password unchanged. Must not set password to empty.
...
None. Ensure password not blanked.

# Notes
Empty password must not overwrite with blank.

# Attachments/Script
```bash
#!/usr/bin/env bash
# Test Case: TC-API-EMP-AUTH-UPDATE-020 - Empty Password
# Endpoint: POST https://seyanty.info/api/employee/update-profile
TITLE="TC-API-EMP-AUTH-UPDATE-020: Empty Password"
echo "=================================================="
echo "Running: $TITLE"
echo "=================================================="
