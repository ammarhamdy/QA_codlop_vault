---
tc_id: TC-API-EMP-AUTH-UPDATE-015
...
Verify server rejects `email` that already belongs to another employee (unique constraint).
...
| email | `employee-08@mail.com` (existing other user) |

# Preconditions
- Valid token for employee-09.
- Another employee exists e.g., `employee-08@mail.com` or `admin-1@mail.com`. If unknown, attempt with `employee-08@mail.com` and document if 422 or 200.
...
- **Notes:** Attempt to update email to duplicate of another user.
...
curl --silent --write-out "\nHTTP_STATUS:%{http_code}" --location --request POST 'https://seyanty.info/api/employee/update-profile' \
  ...
  --form 'email="employee-08@mail.com"' \
...
# Steps
1. Login.
2. POST with `email=employee-08@mail.com` duplicate.
3. Verify 422 duplicate error.

# Expected Result
- HTTP `422` with message like `البريد الإلكتروني مستخدم` or `email has already been taken`.
- Data not updated.
...
# Notes
Unique email enforcement – critical.

# Attachments/Script
```bash
#!/usr/bin/env bash
# Test Case: TC-API-EMP-AUTH-UPDATE-015 - Duplicate Email
# Endpoint: POST https://seyanty.info/api/employee/update-profile
TITLE="TC-API-EMP-AUTH-UPDATE-015: Duplicate Email"
echo "=================================================="
echo "Running: $TITLE"
echo "=================================================="
