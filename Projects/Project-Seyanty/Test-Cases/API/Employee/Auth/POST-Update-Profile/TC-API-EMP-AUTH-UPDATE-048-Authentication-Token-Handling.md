---
tc_id: TC-API-EMP-AUTH-UPDATE-048
...
Verify update response `data.token` handling – new token may be rotated and must be usable.
...
| Expected token | string JWT present in `data.token` |
...
- **Notes:** Valid update, check token presence and usability.
...
RESPONSE=$(curl --silent --location --request POST 'https://seyanty.info/api/employee/update-profile' \
...
if [ "$NEW_TOKEN" != "null" ] && [ -n "$NEW_TOKEN" ]; then
  echo "Testing new token on profile..."
  curl --silent --write-out "\nHTTP_STATUS:%{http_code}" --location --request GET 'https://seyanty.info/api/employee/profile' \
    --header "Authorization: Bearer $NEW_TOKEN" \
    --header 'Accept: application/json' \
    --header 'Host: seyanty.info' | head -n 20
else
  echo "No token rotated; old token should still work"
  curl --silent --write-out "\nHTTP_STATUS:%{http_code}" --location --request GET 'https://seyanty.info/api/employee/profile' \
    --header "Authorization: Bearer $TOKEN" \
    --header 'Accept: application/json' \
    --header 'Host: seyanty.info' | head -n 20
...
# Steps
1. Login, POST update, capture `data.token`.
2. If `data.token` present and non-null, test it on GET profile → expect 200.
3. If `null` or missing, old token should still be 200.
4. Document rotation behavior.

# Expected Result
- `data.token` either new JWT (string, `eyJ...`) and usable on profile, or `null`/unchanged with old token still valid. Must not return invalid token.
...
Store new token if rotated for subsequent tests; otherwise keep old.

# Notes
Token rotation after update – security best practice.

# Attachments/Script
```bash
#!/usr/bin/env bash
# Test Case: TC-API-EMP-AUTH-UPDATE-048 - Authentication Token Handling
# Endpoint: POST https://seyanty.info/api/employee/update-profile
TITLE="TC-API-EMP-AUTH-UPDATE-048: Authentication Token Handling"
echo "=================================================="
echo "Running: $TITLE"
echo "=================================================="
