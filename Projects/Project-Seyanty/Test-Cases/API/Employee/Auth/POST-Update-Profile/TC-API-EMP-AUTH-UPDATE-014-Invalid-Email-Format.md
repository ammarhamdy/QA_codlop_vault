---
tc_id: TC-API-EMP-AUTH-UPDATE-014
...
Verify validation when `email` has invalid format (no @, no domain).
...
- **Notes:** Send `email=invalid-email-format`.
...
# Steps
1. Login.
2. POST with malformed email.
3. Verify 422.

# Expected Result
- HTTP `422`. Body contains validation error for email format.
- No update.
...
```bash
#!/usr/bin/env bash
# Test Case: TC-API-EMP-AUTH-UPDATE-014 - Invalid Email Format
# Endpoint: POST https://seyanty.info/api/employee/update-profile
TITLE="TC-API-EMP-AUTH-UPDATE-014: Invalid Email Format"
echo "=================================================="
echo "Running: $TITLE"
echo "=================================================="
