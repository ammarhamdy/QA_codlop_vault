---
tc_id: TC-API-EMP-AUTH-UPDATE-026
title: Invalid Profile Photo Format
priority:
  - Medium
status:
type:
...
Verify server rejects non-image file as photo (e.g., .txt, .pdf).
...
| photo | `@/tmp/invalid_profile.txt` (text/plain) |

# Preconditions
- Valid token.
- Invalid file `/tmp/invalid_profile.txt` exists.
...
- **Notes:** Send `photo=@/tmp/invalid_profile.txt`.
- **Complete executable cURL for this scenario:**
  ```bash
echo "not an image" > /tmp/invalid_profile.txt
...
curl --silent --write-out "\nHTTP_STATUS:%{http_code}" --location --request POST 'https://seyanty.info/api/employee/update-profile' \
  ...
  --form 'job_title="eng"' \
  --form 'photo=@"/tmp/invalid_profile.txt"'
  ```

# Steps
1. Create txt file.
2. Login, POST with txt as photo.
3. Verify 422 validation error for photo mime.

# Expected Result
- HTTP `422` with message indicating photo must be image (mimes: jpeg,png,jpg).
- Not 500.
...
Delete `/tmp/invalid_profile.txt`.
...
```bash
#!/usr/bin/env bash
# Test Case: TC-API-EMP-AUTH-UPDATE-026 - Invalid Profile Photo Format
# Endpoint: POST https://seyanty.info/api/employee/update-profile
TITLE="TC-API-EMP-AUTH-UPDATE-026: Invalid Profile Photo Format"
echo "=================================================="
echo "Running: $TITLE"
echo "=================================================="

RESPONSE=$(echo "not an image" > /tmp/invalid_profile.txt
TOKEN=$(curl --silent --location --request POST 'https://seyanty.info/api/employee/login' \
  --header 'Accept: application/json' \
  --form 'email_or_name="employee-09@mail.com"' \
  --form 'password="Admin#123"' | jq -r '.data.token')
curl --silent --write-out "\nHTTP_STATUS:%{http_code}" --location --request POST 'https://seyanty.info/api/employee/update-profile' \
  ...
  --form 'phone="0500000109"' \
  --form 'job_title="eng"' \
  --form 'photo=@"/tmp/invalid_profile.txt"' 2>&1)
