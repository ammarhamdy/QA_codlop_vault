---
tc_id: TC-API-EMP-AUTH-UPDATE-046
title: Employee Status Remains Unchanged
priority:
  - Medium
status:
  - completed
...
Verify `data.status` (inactive) remains unchanged after update – status not updatable via this endpoint.
...
- **Notes:** Valid update, check status unchanged.
...
curl --silent --location --request POST 'https://seyanty.info/api/employee/update-profile' \
  ...
  --form 'name="employee-09-statustest"' \
  --form 'email="employee-09@mail.com"' \
  --form 'password="Admin#123"' \
  --form 'phone="0500000109"' \
  --form 'job_title="eng"' | jq -r '.data.status'
  ```

# Steps
1. Login.
2. POST update.
3. Assert `data.status == "inactive"`.
4. GET profile confirms same.

# Expected Result
- Status remains `inactive` (not `active` or null).
- Client cannot escalate status via update-profile.
...
# Notes
Status immutability – security.

# Attachments/Script
```bash
#!/usr/bin/env bash
# Test Case: TC-API-EMP-AUTH-UPDATE-046 - Employee Status Remains Unchanged
# Endpoint: POST https://seyanty.info/api/employee/update-profile
TITLE="TC-API-EMP-AUTH-UPDATE-046: Employee Status Remains Unchanged"
echo "=================================================="
echo "Running: $TITLE"
echo "=================================================="
...
curl --silent --location --request POST 'https://seyanty.info/api/employee/update-profile' \
  ...
  --form 'password="Admin#123"' \
  --form 'phone="0500000109"' \
  --form 'job_title="eng"' | jq -r '.data.status' 2>&1)
