---
tc_id: TC-API-EMP-AUTH-UPDATE-027
title: Invalid Profile Photo Size
priority:
  - Medium
status:
type:
...
Verify server rejects oversized photo (>2MB or > server limit).
...
| photo | `@/tmp/large_profile.jpg` (oversized, >2MB) |

# Preconditions
- Valid token.
- Large file `/tmp/large_profile.jpg` >2MB created via dd/truncate.
...
- **Notes:** Create 3MB dummy file and send as photo.
...
# Create 3MB file if not exists
[ -f /tmp/large_profile.jpg ] || dd if=/dev/urandom of=/tmp/large_profile.jpg bs=1M count=3 2>/dev/null || truncate -s 3M /tmp/large_profile.jpg
...
curl --silent --write-out "\nHTTP_STATUS:%{http_code}" --location --request POST 'https://seyanty.info/api/employee/update-profile' \
  ...
  --form 'job_title="eng"' \
  --form 'photo=@"/tmp/large_profile.jpg"'
  ```

# Steps
1. Create oversized file.
2. Login, POST with large photo.
3. Verify 422 size validation.

# Expected Result
- HTTP `422` with message photo max size exceeded (e.g., `max:2048`).
...
Delete `/tmp/large_profile.jpg` (or keep truncated).
...
```bash
#!/usr/bin/env bash
# Test Case: TC-API-EMP-AUTH-UPDATE-027 - Invalid Profile Photo Size
# Endpoint: POST https://seyanty.info/api/employee/update-profile
TITLE="TC-API-EMP-AUTH-UPDATE-027: Invalid Profile Photo Size"
echo "=================================================="
echo "Running: $TITLE"
echo "=================================================="
...
curl --silent --write-out "\nHTTP_STATUS:%{http_code}" --location --request POST 'https://seyanty.info/api/employee/update-profile' \
  ...
  --form 'phone="0500000109"' \
  --form 'job_title="eng"' \
  --form 'photo=@"/tmp/large_profile.jpg"' 2>&1)
