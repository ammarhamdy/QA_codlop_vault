---
tc_id: TC-API-EMP-AUTH-UPDATE-040
...
type:
  - API
linked_requirement: REQ-EMP-AUTH-UPDATE-019
tags:
  - test-case
  - api
  - employee
...
Validate full success response JSON schema and types for update-profile.
...
- **Notes:** Standard valid POST with multipart.
...
# Steps
1. Login and POST update.
2. Parse JSON with jq.
3. Validate top-level and data keys.

# Expected Result
- HTTP `200`.
- Top-level: `status:"success"`, `code:200`, `message:"تم تحديث البيانات بنجاح"`, `data` object.
- `data` keys: `id` int, `name` string, `email` string, `phone` string, `photo` string URL, `job_title` string, `overview` string|null, `token` string (JWT), `status` string, `orders_count` null|int.
- Validate via `jq -e '.status=="success" and .code==200 and (.data|has("token"))'`.
...
# Notes
Schema includes token unlike profile GET.

# Attachments/Script
```bash
#!/usr/bin/env bash
# Test Case: TC-API-EMP-AUTH-UPDATE-040 - Success Response Body Structure
# Endpoint: POST https://seyanty.info/api/employee/update-profile
TITLE="TC-API-EMP-AUTH-UPDATE-040: Success Response Body Structure"
echo "=================================================="
echo "Running: $TITLE"
echo "=================================================="
