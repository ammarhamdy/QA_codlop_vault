---
tc_id: TC-API-EMP-AUTH-UPDATE-008
...
type:
  - API
linked_requirement: REQ-EMP-AUTH-UPDATE-008
tags:
  - test-case
  - api
  - employee
...
Verify `overview` text field update persists.
...
| overview | `updated overview text for testing` |
| name | `employee-09` |
| email | `employee-09@mail.com` |

# Preconditions
- Valid token.
- New overview `updated overview text for testing`.
...
curl --silent --write-out "\nHTTP_STATUS:%{http_code}" --location --request POST 'https://seyanty.info/api/employee/update-profile' \
  ...
  --form 'job_title="eng"' \
  --form 'overview="updated overview text for testing"'
  ```

# Steps
1. Login.
2. POST with new overview.
3. Verify `data.overview` equals new text.
...
Revert to `experienced engineer` or `null` depending on original; set back to `experienced engineer`.

# Notes
Overview is nullable text area.

# Attachments/Script
```bash
#!/usr/bin/env bash
# Test Case: TC-API-EMP-AUTH-UPDATE-008 - Update Overview
# Endpoint: POST https://seyanty.info/api/employee/update-profile
TITLE="TC-API-EMP-AUTH-UPDATE-008: Update Overview"
echo "=================================================="
echo "Running: $TITLE"
echo "=================================================="
...
curl --silent --write-out "\nHTTP_STATUS:%{http_code}" --location --request POST 'https://seyanty.info/api/employee/update-profile' \
  ...
  --form 'phone="0500000109"' \
  --form 'job_title="eng"' \
  --form 'overview="updated overview text for testing"' 2>&1)
