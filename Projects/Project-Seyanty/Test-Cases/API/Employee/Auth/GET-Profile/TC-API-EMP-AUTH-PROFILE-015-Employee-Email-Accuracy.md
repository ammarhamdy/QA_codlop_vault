---
tc_id: TC-API-EMP-AUTH-PROFILE-015
title: Employee Email Accuracy
priority:
  - Medium
status:
type:
...
Verify `data.email` is correct and properly formatted.
...
| ----- | ----- |
| Endpoint | `GET https://seyanty.info/api/employee/profile` |
| Expected email | `employee-09@mail.com` |
...
curl --silent --location --request GET 'https://seyanty.info/api/employee/profile' \
  ...
  --header 'Accept: application/json' \
  --header 'Host: seyanty.info' | jq -r '.data.email'
  ```

# Steps
1. GET profile.
2. Assert email equals `employee-09@mail.com` and matches regex `[^@]+@[^@]+\.[^@]+`.

# Expected Result
- `data.email == "employee-09@mail.com"`.
- Valid email format, lowercase normalization.
...
```bash
#!/usr/bin/env bash
# Test Case: TC-API-EMP-AUTH-PROFILE-015 - Employee Email Accuracy
# Endpoint: GET https://seyanty.info/api/employee/profile
TITLE="TC-API-EMP-AUTH-PROFILE-015: Employee Email Accuracy"
echo "=================================================="
echo "Running: $TITLE"
echo "=================================================="
...
curl --silent --location --request GET 'https://seyanty.info/api/employee/profile' \
  --header "Authorization: Bearer $TOKEN" \
  --header 'Accept: application/json' \
  --header 'Host: seyanty.info' | jq -r '.data.email' 2>&1)
