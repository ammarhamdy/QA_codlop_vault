---
tc_id: TC-API-EMP-AUTH-PROFILE-017
title: Employee Photo Accuracy
priority:
  - Low
status:
  - completed
...
Verify `data.photo` is a valid HTTPS URL pointing to storage, not empty or broken.
...
| ----- | ----- |
| Endpoint | `GET https://seyanty.info/api/employee/profile` |
| Expected photo | `https://seyanty.info/storage/employees/zy0RHyVmKiz1KoWP9jsxRfrIpd4qL8BRNLJ0hRkA.jpg` or any `https://seyanty.info/storage/employees/*.jpg` |

# Preconditions
- Valid token.
- Employee-09 has photo seeded.
...
- **Notes:** Valid GET; optionally HEAD the photo URL to verify 200.
...
PHOTO=$(curl --silent --location --request GET 'https://seyanty.info/api/employee/profile' \
  --header "Authorization: Bearer $TOKEN" \
  --header 'Accept: application/json' \
  --header 'Host: seyanty.info' | jq -r '.data.photo')
echo "PHOTO:$PHOTO"
curl --silent --write-out "\nHTTP_STATUS:%{http_code}" --location --request HEAD "$PHOTO" | head -n 20
  ```

# Steps
1. GET profile, extract `data.photo`.
2. Assert starts with `https://seyanty.info/storage/employees/`.
3. Optionally HEAD that URL → expect 200.
4. Ensure not null if employee has photo.

# Expected Result
- `data.photo` is non-empty string URL with `https` scheme.
- Contains `/storage/employees/` path.
- HEAD returns 200 (image reachable).
...
# Notes
Photo may change if updated via profile update; pattern check is sufficient.

# Attachments/Script
```bash
#!/usr/bin/env bash
# Test Case: TC-API-EMP-AUTH-PROFILE-017 - Employee Photo Accuracy
# Endpoint: GET https://seyanty.info/api/employee/profile
TITLE="TC-API-EMP-AUTH-PROFILE-017: Employee Photo Accuracy"
echo "=================================================="
echo "Running: $TITLE"
echo "=================================================="
...
PHOTO=$(curl --silent --location --request GET 'https://seyanty.info/api/employee/profile' \
  ...
  --header 'Host: seyanty.info' | jq -r '.data.photo')
echo "PHOTO:$PHOTO"
curl --silent --write-out "\nHTTP_STATUS:%{http_code}" --location --request HEAD "$PHOTO" | head -n 20 2>&1)
