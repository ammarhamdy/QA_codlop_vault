---
tc_id: TC-API-EMP-AUTH-PROFILE-018
title: Employee Job Title Accuracy
priority:
  - Low
status:
type:
...
Verify `data.job_title` reflects stored job title.
...
curl --silent --location --request GET 'https://seyanty.info/api/employee/profile' \
  ...
  --header 'Accept: application/json' \
  --header 'Host: seyanty.info' | jq -r '.data.job_title'
  ```

# Steps
1. GET profile.
2. Assert `data.job_title == "engineer"`.

# Expected Result
- `data.job_title` is `engineer` (lowercase, per seed).
...
# Notes
Job title is display field.

# Attachments/Script
```bash
#!/usr/bin/env bash
# Test Case: TC-API-EMP-AUTH-PROFILE-018 - Employee Job Title Accuracy
# Endpoint: GET https://seyanty.info/api/employee/profile
TITLE="TC-API-EMP-AUTH-PROFILE-018: Employee Job Title Accuracy"
echo "=================================================="
echo "Running: $TITLE"
echo "=================================================="
...
curl --silent --location --request GET 'https://seyanty.info/api/employee/profile' \
  --header "Authorization: Bearer $TOKEN" \
  --header 'Accept: application/json' \
  --header 'Host: seyanty.info' | jq -r '.data.job_title' 2>&1)
