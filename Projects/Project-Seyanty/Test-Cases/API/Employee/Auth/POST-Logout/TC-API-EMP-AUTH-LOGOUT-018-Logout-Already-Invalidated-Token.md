---
tc_id: TC-API-EMP-AUTH-LOGOUT-018
title: Logout Already Invalidated Token
priority:
  - Medium
status:
  - completed
...
Verify second logout attempt with already-invalidated token returns 401, not 200 or 500.
...
| Field | Value |
| ----- | ----- |
| Endpoint | `POST https://seyanty.info/api/employee/logout` twice with same token |

# Preconditions
- Token already logged out once.
...
- **Notes:** Login, logout, then immediate second logout same token.
...
# Steps
1. Login.
2. First logout → expect 200.
3. Second logout same token → expect 401.

# Expected Result
- First: `200` `تم تسجيل الخروج بنجاح`.
- Second: `401` `لابد من تسجيل الدخول أولا`.
- No 500.
...
# Notes
Idempotency check – logout not idempotent.

# Attachments/Script
```bash
#!/usr/bin/env bash
# Test Case: TC-API-EMP-AUTH-LOGOUT-018 - Logout Already Invalidated Token
# Endpoint: POST https://seyanty.info/api/employee/logout
TITLE="TC-API-EMP-AUTH-LOGOUT-018: Logout Already Invalidated Token"
echo "=================================================="
echo "Running: $TITLE"
echo "=================================================="
