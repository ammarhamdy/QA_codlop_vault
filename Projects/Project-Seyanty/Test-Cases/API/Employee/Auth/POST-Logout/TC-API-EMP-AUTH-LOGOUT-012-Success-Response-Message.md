---
tc_id: TC-API-EMP-AUTH-LOGOUT-012
...
Verify success message is exactly Arabic `تم تسجيل الخروج بنجاح`.
...
| Expected message | `تم تسجيل الخروج بنجاح` |
...
- **Notes:** Valid POST logout; check message field.
...
curl --silent --location --request POST 'https://seyanty.info/api/employee/logout' \
  ...
  --header 'Accept: application/json' \
  --header 'Host: seyanty.info' | jq -r '.message'
  ```

# Steps
1. POST logout.
2. Extract `.message`.
3. Assert exact Arabic string.

# Expected Result
- `message == "تم تسجيل الخروج بنجاح"`.
- No extra whitespace or Latin translation.
...
# Notes
Localization check – must match spec exactly.

# Attachments/Script
```bash
#!/usr/bin/env bash
# Test Case: TC-API-EMP-AUTH-LOGOUT-012 - Success Response Message
# Endpoint: POST https://seyanty.info/api/employee/logout
TITLE="TC-API-EMP-AUTH-LOGOUT-012: Success Response Message"
echo "=================================================="
echo "Running: $TITLE"
echo "=================================================="
...
curl --silent --location --request POST 'https://seyanty.info/api/employee/logout' \
  --header "Authorization: Bearer $TOKEN" \
  --header 'Accept: application/json' \
  --header 'Host: seyanty.info' | jq -r '.message' 2>&1)
