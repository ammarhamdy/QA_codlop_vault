---
tc_id: TC-API-DASH-ADMIN-CREATE-001
title: Create a new admin with valid data
priority:
  - High
status:
  - completed
...
4. Add form fields `name`, `phone`, `email`, `password`, and `photo`.
5. Send the request and capture the response.

# Expected Result
- The API returns HTTP status code `200 OK` or `201 Created`.
- Response body:
```json
{
    "status": "success",
...
```
- The new admin is created successfully and the returned `data` matches the submitted input.

# Notes
- Use a unique email for each run to avoid `Duplicate-Email` errors.
