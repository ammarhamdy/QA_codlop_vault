---
tc_id: TC-API-DASH-ADMIN-UPDATE-002
title: Update admin name
priority:
  - High
status:
  - completed
...
| _method | `put` |
| name | `admin-1` (new name) |
| email | `admin-7@mail.com` (unchanged) |
| photo | *(unchanged)* |
...
4. Add form fields `_method=put` and a new `name` value.
5. Send the request and capture the response.

# Expected Result
- The API returns HTTP status code `200 OK` with `"تم تعديل البيانات بنجاح"`.
- `data.name` equals the new name (`admin-1`).
- Only the `name` field changes; `email`, `photo`, and `is_blocked` remain unchanged.

# Notes
- Cleanup: restore the original name after the test.
