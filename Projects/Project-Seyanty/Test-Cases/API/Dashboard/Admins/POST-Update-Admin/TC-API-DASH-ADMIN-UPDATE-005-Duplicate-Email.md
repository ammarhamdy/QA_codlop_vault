---
tc_id: TC-API-DASH-ADMIN-UPDATE-005
title: Update admin with a duplicate email
priority:
  - High
status:
  - completed
...
| _method | `put` |
| name | `admin-1` |
| email | `admin-7@mail.com` *(already registered to another admin)* |

# Preconditions
- User is logged in to the dashboard as a privileged admin with a valid token.
- An admin record with the given `id` exists.
- The target email is already registered to a different admin.
...
4. Add form fields with an email that already belongs to another admin.
...
- The admin record is NOT updated.

# Notes
- Unless the target admin itself owns the email, duplicates must be rejected.
