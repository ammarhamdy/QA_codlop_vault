---
tc_id: TC-API-DASH-ADMIN-UPDATE-004
...
4. Add form fields `_method=put`, `name`, and the new `email` value.
5. Send the request and capture the response.

# Expected Result
- The API returns HTTP status code `200 OK` with `"تم تعديل البيانات بنجاح"`.
- `data.email` equals the new email (`admin-7@mail.com`).
- Only the `email` field changes; other fields remain unchanged.

# Notes
- Cleanup: restore the original email after the test.
